<?php
  ...
  $iii=$_POST['idscr'];
  $string=strval($id)."|".strval($name)."|".strval($deptno)."|".strval($part)."|".strval($ach)."#";
  $ii=strval($id);
  ...
  while(!feof($file1)){
    ...
    if(preg_match("/". preg_quote($ii,NULL) . "/",$line1)){
            ...
            $i=strval($ext1)."|".strval($ext2)."|".strval($ext3)."|".strval($ext4)."|".strval($ext5)."#";
          }
        }
      fclose($file1);

  $b=file_get_contents("insert_achievement_prof.txt");
  $c=preg_replace("/".preg_quote($i)."/",$string,$b);
  file_put_contents("insert_achievement_prof.txt",$c);
